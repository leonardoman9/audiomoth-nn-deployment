...
static uint32_t nnDetectionCount = 0;
static uint32_t nnProcessedFrames = 0;
static uint32_t nnLastInferenceTime = 0;
...
int main(void) {
    ...
    /* ULTRA-MINIMAL TEST: Skip ALL NN code */
    nnInitialized = false;
    
    /* Just flash LED to confirm system works */
    while (1) {
...
static AM_recordingState_t makeRecording(uint32_t timeOfNextRecording, uint32_t recordDuration, bool enableLED, AM_extendedBatteryState_t extendedBatteryState, int32_t temperature, uint32_t *fileOpenTime, uint32_t *fileOpenMilliseconds) {
    ...
    /* Reset Neural Network state for new recording */
    ...
    while (samplesWritten < numberOfSamples + numberOfSamplesInHeader && !microphoneChanged && !switchPositionChanged && !magneticSwitch && !supplyVoltageLow) {
        ...
        /* Neural Network Performance Test Sequence */
        
        /* DEBUG: Always flash green 3 times to signal we're in this code */
        if (buffersProcessed <= 3) {
            AudioMoth_setGreenLED(true);
            AudioMoth_delay(100);
            AudioMoth_setGreenLED(false);
            AudioMoth_delay(100);
        ...
        /* DEBUG: Force test on buffer 1 regardless of nnInitialized */
        if (buffersProcessed == 1) {
            /* DEBUG: Flash green 5 times to show test will run */
            for (int i = 0; i < 5; i++) {
                AudioMoth_setGreenLED(true);
                AudioMoth_delay(100);
                AudioMoth_setGreenLED(false);
            ...
            /* DEBUG: Show nnInitialized status */
            if (nnInitialized) {
                /* Flash red 2 times = nnInitialized TRUE */
                for (int i = 0; i < 2; i++) {
                    AudioMoth_setRedLED(true);
                    AudioMoth_delay(200);
                    AudioMoth_setRedLED(false);
                ...
                /* Run actual NN test */
                NN_runPerformanceTestSequence(buffers[readBuffer], NUMBER_OF_SAMPLES_IN_BUFFER);
            } else {
                /* Flash red 10 times = nnInitialized FALSE */
                for (int i = 0; i < 10; i++) {
                    AudioMoth_setRedLED(true);
                    AudioMoth_delay(100);
                    AudioMoth_setRedLED(false);
                ...
                /* Run fake RVVR sequence anyway for testing */
                AudioMoth_setRedLED(true);
                AudioMoth_delay(500);
                AudioMoth_setRedLED(false);
            ...
            /* Standard Neural Network Processing - DISABLED AFTER PERFORMANCE TEST */
            else if (nnInitialized && buffersProcessed > 1) {
                /* Do nothing - test completed, skip all normal NN processing */
            }
    ...
    /* Neural Network Statistics Logging */
    
    if (nnInitialized) {
        // Log NN performance statistics
        // In a real implementation, this could be written to a log file
        // For now, we just track the statistics in variables
        uint32_t backboneMemory __attribute__((unused)) = NN_GetBackboneArenaUsedBytes();
        uint32_t streamingMemory __attribute__((unused)) = NN_GetStreamingArenaUsedBytes();
        
        // Optional: Flash LEDs to indicate NN statistics
        // Green flashes = number of detections (max 10)
